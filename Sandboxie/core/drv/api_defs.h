...
API_ARGS_FIELD(ULONG, source_tid)
API_ARGS_CLOSE(API_MONITOR_PUT2_ARGS)

#define API_TRACE_NAME_MAX             96
#define API_TRACE_BATCH_MAX_RECORDS   128

typedef struct _API_TRACE_BATCH_RECORD {
    ULONG ThreadId;
    ULONG Type;
    USHORT NameLength;
    WCHAR Name[API_TRACE_NAME_MAX];
} API_TRACE_BATCH_RECORD;

API_ARGS_BEGIN(API_MONITOR_PUT_BATCH_ARGS)
API_ARGS_FIELD(ULONG, record_count)
API_ARGS_FIELD(API_TRACE_BATCH_RECORD *, records)
API_ARGS_FIELD(ULONG, buffer_len)
API_ARGS_CLOSE(API_MONITOR_PUT_BATCH_ARGS)
