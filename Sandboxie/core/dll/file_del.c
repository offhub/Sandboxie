...
_FX BOOLEAN File_CompactPathJournal_internal(CRITICAL_SECTION* CritSec, PATH_LIST* Root, const WCHAR* datName, const WCHAR* logName, WCHAR* (*TranslatePath)(const WCHAR*), WCHAR* (*TranslatePathForLoad)(const WCHAR*), ULONG64* pLogSize, ULONG64* pLogDate, ULONG64* pReplayOffset, HANDLE* pAppendHandle, BOOLEAN isV3)
{
    ...
    if (bSerializedOK && !compactBuf.OOM) {
        ...
        // File_InitDelete_v2 and Key_InitDelete_v2 detect a stale .dat.bak at
        // startup and restore from it when the .dat is empty or missing.
...
_FX BOOLEAN File_LoadPathTree()
{
    ...
    if (File_DeleteV3) {
        ...
        ULONG64 dummy_offset = 0;
        // FALSE means the .dat does not exist yet (fresh sandbox; File_PathsFileSize == 0)
        // or a transient I/O error.  If File_PathsFileSize > 0 we had authoritative data
        // before; bail out below so RefreshPathTree does not advance the freshness cache
        // and retries on the next poll instead of locking in an empty tree.
        BOOLEAN datOk = File_LoadPathJournal_internal(&File_PathRoot, datName, File_TranslateDosToNtPathForDatFile, &dummy_offset, NULL);

        // V3: journal (.sbie) is always active; replay it on top of the .dat snapshot.
        // Return value not checked: FALSE means .sbie did not exist yet (normal right
        // after compaction) or a transient I/O error.  In either case the tree stays at
        // the .dat snapshot; missing journal entries are replayed as soon as the next
        // write to the .sbie changes its size or date and triggers an incremental refresh.
        ...
        // Transient .dat I/O error: bail out without advancing the freshness cache so
        // RefreshPathTree retries.  The tree is already cleared; it will be rebuilt on
        // the next successful reload.
        if (!datOk && File_PathsFileSize > 0) {
            LeaveCriticalSection(File_PathRoot_CritSec);
...
//#endif
    ...
    // V3 .dat.bak recovery: if a stale backup exists from a failed compaction write,
    // use it to recover a missing/empty .dat (File_PathsFileSize == 0).  Either way,
    // remove the stale .bak once .dat is confirmed non-empty.
    if (File_DeleteV3) {
        ULONG bakLen = (ULONG)wcslen(FILE_PATH_FILE_NAME_V3) + 5; // ".bak\0"
        WCHAR* bakName = (WCHAR*)Dll_Alloc(bakLen * sizeof(WCHAR));
        if (bakName) {
            wcscpy(bakName, FILE_PATH_FILE_NAME_V3);
            wcscat(bakName, L".bak");
            ULONG64 bakSize = 0, bakDate = 0;
            if (File_GetAttributes_internal(bakName, &bakSize, &bakDate, NULL)) {
                if (File_PathsFileSize == 0 && bakSize > 0) {
                    // .dat is missing/empty: restore from backup and reload.
                    if (File_CopyDataFile(bakName, FILE_PATH_FILE_NAME_V3)) {
                        File_LoadPathTree();
                        File_GetAttributes_internal(FILE_PATH_FILE_NAME_V3, &File_PathsFileSize, &File_PathsFileDate, NULL);
                    }
                }
                // Remove the .bak once .dat is non-empty (restored or already intact).
                if (File_PathsFileSize > 0)
                    File_DeleteDataFile(bakName);
            }
            Dll_Free(bakName);
        }
    }
