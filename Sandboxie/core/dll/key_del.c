...
BOOLEAN File_CopyDataFile(const WCHAR* sourceName, const WCHAR* targetName);
VOID File_DeleteDataFile(const WCHAR* name);
...
_FX BOOLEAN Key_LoadPathTree()
{
    ...
    if (Key_DeleteV3) {
        ...
        // FALSE means the .dat does not exist yet (fresh sandbox; Key_PathsFileSize == 0)
        // or a transient I/O error.  If Key_PathsFileSize > 0 we had authoritative data
        ...
        BOOLEAN datOk = File_LoadPathJournal_internal(&Key_PathRoot, datName, NULL, &dummy_offset, NULL);
        Key_RegPaths_Loaded = datOk;

        // V3: always replay the .sbie journal on top of the .dat snapshot.
...
//#endif
    ...
    // use it to recover a missing/empty .dat (Key_PathsFileSize == 0).  Either way,
    // remove the stale .bak once .dat is confirmed non-empty.
    if (Key_DeleteV3) {
        ULONG bakLen = (ULONG)wcslen(KEY_PATH_FILE_NAME_V3) + 5; // ".bak\0"
        WCHAR* bakName = (WCHAR*)Dll_Alloc(bakLen * sizeof(WCHAR));
        if (bakName) {
            wcscpy(bakName, KEY_PATH_FILE_NAME_V3);
            wcscat(bakName, L".bak");
            ULONG64 bakSize = 0, bakDate = 0;
            if (File_GetAttributes_internal(bakName, &bakSize, &bakDate, NULL)) {
                if (Key_PathsFileSize == 0 && bakSize > 0) {
                    // .dat is missing/empty: restore from backup and reload.
                    if (File_CopyDataFile(bakName, KEY_PATH_FILE_NAME_V3)) {
                        Key_LoadPathTree();
                        File_GetAttributes_internal(KEY_PATH_FILE_NAME_V3, &Key_PathsFileSize, &Key_PathsFileDate, NULL);
                    }
                }
                // Remove the .bak once .dat is non-empty (restored or already intact).
                if (Key_PathsFileSize > 0)
