...
BOOLEAN File_OpenDataFile(const WCHAR* name, HANDLE* hPathsFile, BOOLEAN Append, ULONG64* pEndOfFile);
BOOLEAN File_OpenDataFileAppendShared(const WCHAR* name, HANDLE* hPathsFile, ULONG64* pEndOfFile);
BOOLEAN File_SeekToEndOfFile(HANDLE fileHandle, ULONG64* pEndOfFile);
...
_FX BOOLEAN Key_SavePathTree()
{
    ...
    if (Key_DeleteV3) {
        ...
        bSaved = File_SavePathTree_internal(&Key_PathRoot, datName, NULL);
    }

    // Only update the freshness cache when the write succeeded.  See File_SavePathTree for
    // the full rationale; the same File_OpenDataFile / FILE_OVERWRITE_IF truncation hazard
    // applies here for the registry VCM dat file.
    if (bSaved)
    ...
    // Return TRUE unconditionally: no caller checks this value, see File_SavePathTree.
    return TRUE;
}
...
_FX BOOLEAN Key_LoadPathTree()
{
    ...
    if (Key_DeleteV3) {
        ...
        BOOLEAN datOk = !datExists || datSize == 0 ||
            File_LoadPathJournal_internal(&newRoot, datName, NULL, &dummy_offset, NULL, FALSE);
        ...
        BOOLEAN logOk = !logExists || logSize == 0 ||
            File_LoadPathJournal_internal(&newRoot, logName, NULL, &replayOffset, &applied_lines, TRUE);
        if (!datOk || !logOk) {
        ...
        PATH_LIST oldRoot = Key_PathRoot;
        Key_PathRoot = newRoot;
        File_ClearPathBranche_internal(&oldRoot);
        Key_PathsLogReplayOffset = replayOffset;
        Key_PathsLogLineCount = applied_lines;
        Key_RegPaths_Loaded = (datExists && datSize > 0) || applied_lines > 0;
    } else {
        // V2: legacy .dat format only, no journal interaction.
        // Mirror the V3 transient-error guard: if the internal load failed and we
        // previously had data (Key_PathsFileSize > 0), bail out without advancing
        ...
        if (!Key_RegPaths_Loaded && Key_PathsFileSize > 0) {
            LeaveCriticalSection(Key_PathRoot_CritSec);
            ReleaseMutex(Key_VcmMutex);
            return FALSE;
        }
...
_FX VOID Key_RefreshPathTree()
{
    ...
    if (Key_DeleteV3) {
        ...
        boxChanged = File_TestBoxRootChange(1);
    } else if (!File_TestBoxRootChange(1)) {
    ...
    if ((datExists && (Key_PathsFileSize != PathsFileSize || Key_PathsFileDate != PathsFileDate))
        || (!datExists && (Key_PathsFileSize != 0 || Key_PathsFileDate != 0))) {
    ...
    BOOLEAN logExists = Key_DeleteV3
        && File_GetAttributes_internal(logName, &PathsLogFileSize, &PathsLogFileDate, NULL);
    if (Key_DeleteV3
        && ((logExists && (Key_PathsLogFileSize != PathsLogFileSize || Key_PathsLogFileDate != PathsLogFileDate))
            || (!logExists && (Key_PathsLogFileSize != 0 || Key_PathsLogFileDate != 0)))) {
    ...
    if (bReload) {
        ...
        if (bDatChanged || !Key_DeleteV3) {
            ...
            ULONG applied_lines = 0;
            if (!File_LoadPathJournal_internal(&Key_PathRoot, logName, NULL, &Key_PathsLogReplayOffset, &applied_lines, TRUE)) {
                LeaveCriticalSection(Key_PathRoot_CritSec);
                ReleaseMutex(Key_VcmMutex);
                Key_LoadPathTree();
                return;
...
// Key_SetDeleteV3
//
// Must be called only during process initialisation (Key_Init / File_Init),
// before any delete or relocation operations take place.  The mode flag is
// read without synchronisation throughout the hot path; toggling it at
// runtime would cause races between readers that see old vs new values and
// is therefore unsafe.
//---------------------------------------------------------------------------
...
_FX BOOLEAN Key_InitDelete_v2()
{
    ...
    if (!Key_PathRoot_CritSec) return FALSE;
    ...
    {
        Key_VcmMutex = CreateMutex(NULL, FALSE, KEY_VCM_MUTEX);
    }

    if (Key_DeleteV3) {
        // Named page-file-backed section for cross-process burst-deferral (registry journal).
        // Only needed in v3 journal mode.
        Key_SharedBusyHandle = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, sizeof(LONG), KEY_VCM_BUSY_TICK);
        ...
        Key_JournalCtx.pPathsVersion       = &Key_PathsVersion;
        Key_JournalCtx.pIsV3               = &Key_DeleteV3;
    }

    BOOLEAN pathsLoaded = Key_LoadPathTree(); // sets Key_PathsLogReplayOffset and Key_PathsLogLineCount
...
//#endif
    ...
    return !Key_DeleteV3 || pathsLoaded;
}
...
_FX NTSTATUS Key_CompactDeleteV3(void)
{
    if (!Key_DeleteV3 || !Key_VcmMutex || !Key_PathRoot_CritSec)
    ...
    BOOLEAN compacted = Vfs_CompactJournalNow_internal(&Key_JournalCtx);
    ReleaseMutex(Key_VcmMutex);
...
_FX NTSTATUS Key_MarkDeletedEx_v2(const WCHAR* TruePath, const WCHAR* ValueName)
{
    ...
    // Note: no trailing-backslash normalization is applied here.  Registry NT
    // paths (e.g. \REGISTRY\MACHINE\SOFTWARE\Foo) never carry a trailing
    // backslash from callers, so the File_TrimTrailingBackslashes step that
    // File_MarkDeleted_v2 performs for file paths is not needed.

    if (Key_DeleteV3) {
        BOOLEAN applied = FALSE;
        BOOLEAN persisted = Vfs_AppendAndMaybeCompact_internal(&Key_JournalCtx,
            TRUE, ValueName ? TruePath : FullPath, ValueName, &applied);
        ReleaseMutex(Key_VcmMutex);
    ...
    // bTruncated covers the OOM case: see File_MarkDeleted_internal for details.
    if (bSet || bTruncated) 
    {
        // Do NOT bump Key_PathsVersion here.  Per-delete invalidation can reset
        // in-progress registry merge state and make a single delete operation
        // fan out to sibling entries.  Keep version bumps tied to full
        // reload/replay paths only (Key_LoadPathTree/Key_RefreshPathTree).
        //
        ...
        if (!bTruncated 
            ...
            HANDLE hPathsFile;
            if (File_OpenDataFile(KEY_PATH_FILE_NAME, &hPathsFile, TRUE, NULL))
            {
                ...
                Key_SavePathTree(); // open-for-append failed; fall back to full rewrite
        }
        else
            Key_SavePathTree();
    }
...
_FX ULONG Key_IsDeletedEx_v2(const WCHAR* TruePath, const WCHAR* ValueName, BOOLEAN IsValue)
{
    ...
    if (!FullPath) return 0; // treat as not-deleted on OOM
...
_FX NTSTATUS Key_SetRelocation(const WCHAR *OldTruePath, const WCHAR *NewTruePath)
{
    ...
    if (Key_DeleteV3) {
        BOOLEAN applied = FALSE;
        BOOLEAN persisted = Vfs_AppendAndMaybeCompact_internal(&Key_JournalCtx,
            FALSE, OldTruePath, NewTruePath, &applied);
        ReleaseMutex(Key_VcmMutex);
        return persisted && applied ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL;
    }
