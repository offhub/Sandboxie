...
extern BOOLEAN DNS_DnsApiHookEnabled;  // FilterDnsApi setting (default: enabled)
extern BOOLEAN DNS_MapIpv4ToIpv6;  // DnsMapIpv4ToIpv6 setting (default: disabled)

// Exclusion resolver mode for NetworkDnsFilterExclude
// - SYS: excluded domains bypass filtering AND use system DNS (default)
// - ENC: excluded domains bypass filtering but still use encrypted DNS for passthrough
typedef enum _DNS_EXCLUDE_RESOLVE_MODE {
    DNS_EXCLUDE_RESOLVE_SYS = 0,
    DNS_EXCLUDE_RESOLVE_ENC = 1,
} DNS_EXCLUDE_RESOLVE_MODE;
...
// Extended exclusion query that also returns the configured resolver mode.
// Returns TRUE if excluded, FALSE if not excluded.
...
// On success, *ppEntries is NULL for block-mode rules or points to the rule's IP_ENTRY list.
// *ppTypeFilter may be NULL if no NetworkDnsFilterType was configured for the entry.
BOOLEAN DNS_TryGetExactFilterMatch(const WCHAR* pszName, LIST** ppEntries, DNS_TYPE_FILTER** ppTypeFilter);

// Match a (lowercased) DNS domain against a pattern list.
// Returns best match length (>0) if matched, 0 otherwise.
// Note: domain buffer may be temporarily modified during matching.
struct _PATTERN;
int WINAPI DNS_DomainMatchPatternList(WCHAR* domain_lwr, SIZE_T domain_len, LIST* pat_list, struct _PATTERN** found_pat);
