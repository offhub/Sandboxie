        ...
        if (Socket_ParseDnsQuery(dns_payload, dns_payload_len, domainName, 256, &qtype)) {
            ...
            if (match_result > 0) {
                ...
                if (response_len == 0) {
                    ...
                    if (DNS_TraceFlag) {
                        WCHAR msg[256];
                        Sbie_snwprintf(msg, 256, L"TCP DNS: Queueing fake response (answered locally, DNS payload only), len=%d bytes",
                            response_len);
                        SbieApi_MonitorPutMsg(MONITOR_DNS, msg);
                    }
            ...
            if (Socket_ParseDnsQuery(dns_payload, dns_payload_len, domainName, 256, &qtype)) {
                ...
                if (match_result > 0) {
                    ...
                    if (response_len == 0) {
                        ...
                        if (DNS_TraceFlag) {
                            WCHAR msg[256];
                            Sbie_snwprintf(msg, 256, L"TCP DNS: WSASend queueing fake response (answered locally, DNS payload only), len=%d bytes",
                                response_len);
                            SbieApi_MonitorPutMsg(MONITOR_DNS, msg);
                        }
    ...
    *found = NULL;
    ...
    while (pat) {
        ...
        if (!wcschr(pat_text, L'*') && !wcschr(pat_text, L'?')) {
            if (DNS_TraceFlag && DNS_DebugFlag) {
                WCHAR msg[512];
                Sbie_snwprintf(msg, 512, L"DNS Hierarchical: Comparing domain '%s' to pattern '%s' (len: %lu vs %lu)", domain_lwr, pat_text, domain_len, pat_len);
                SbieApi_MonitorPutMsg(MONITOR_DNS, msg);
            }
            if (pat_len == domain_len && _wcsnicmp(pat_text, domain_lwr, domain_len) == 0) {
                ...
                *found = pat;
                if (DNS_TraceFlag) {
                    WCHAR msg[512];
                    Sbie_snwprintf(msg, 512, L"DNS Hierarchical: Domain '%s' matched exact pattern '%s'", domain_lwr, pat_text);
                    SbieApi_MonitorPutMsg(MONITOR_DNS, msg);
                }
    ...
    if (best_wildcard_pattern) {
        *found = best_wildcard_pattern;
        if (DNS_TraceFlag) {
            WCHAR msg[512];
            Sbie_snwprintf(msg, 512, L"DNS Hierarchical: Domain '%s' matched pattern '%s' (mid-wildcard support)", domain_lwr, Pattern_Source(best_wildcard_pattern));
            SbieApi_MonitorPutMsg(MONITOR_DNS, msg);
        }
        Dll_Free(domain_copy);
        return 2;
    ...
    while (pat) {
        ...
        if (pat_text && wcslen(pat_text) == 1 && pat_text[0] == L'*') {
            *found = pat;
            if (DNS_TraceFlag) {
                WCHAR msg[512];
                Sbie_snwprintf(msg, 512, L"DNS Hierarchical: Domain '%s' matched catch-all pattern '*'", domain_lwr);
                SbieApi_MonitorPutMsg(MONITOR_DNS, msg);
            }
            Dll_Free(domain_copy);
            return 3;
