    ...
    *out_CopyHandle = NULL;
    ...
    if (Key_Delete_v2) {
        ...
        if (OldTruePath) {
            ...
            // TruePath was redirected to a different underlying path: recompute
            // mp_flags for the actual target path.  The original mp_flags were
            // computed for the logical (post-rename) name, but PATH_IS_WRITE and
            // related decisions must be based on the path we are about to open.
            mp_flags = SbieDll_MatchPath(L'k', TruePath);
        }
    }
